...
// // 表单获取及其功能实现
// var perydiva=document.getElementById('per-y-div').getElementsByTagName('a');
// var rowtabsamonth=document.getElementById('row-tabsa-month');
// var rowtabsayear=document.getElementById('row-tabsa-year');
// //切换年付和月付样式
// for(i=0;i<perydiva.length;i++){
// 	perydiva[0].onclick=function(){
// 		perydiva[0].setAttribute('class','a1onclick per-y-m1 per-year');//添加属性
// 		perydiva[1].setAttribute('class','a2onclick per-y-m1 per-month');
// 		rowtabsayear.style.display='block';
// 		rowtabsamonth.style.display='none';
// 	}
// 	perydiva[1].onclick=function(){
// 		perydiva[0].setAttribute('class','a2onclick per-y-m1 per-year');
// 		perydiva[1].setAttribute('class','a1onclick per-y-m1 per-month');
// 		  rowtabsayear.style.display='none';
// 		  rowtabsamonth.style.display='block';
// 	}
// }
	    ...
	    planbtn1[i].onclick = function() {
	        for (i = 0; i < planbtn1.length; i++) {
	            ...
	            //按照年进行显示金额
	           	...
	           	//标准版套餐价格
	            amountprice.innerHTML = '￥' + pricearr[this.index] + '.00';
	            //还需支付价格
	            amountpriceb.innerHTML = '￥' + pricearr[this.index] + '.00';
	            touming.style.display = 'block';
	            closepricehou.onclick = function(e) {
 ...
 function fukuan(pricearrbian,pricearrjieshou) {
            for (i = 0; i < threepay.length; i++) {
                ...
                threepay[i].onclick = function() {
                    ...
                    message1[this.order].innerHTML = message1nei[this.order] + '支付' + pricearrbian[pricearrjieshou] + '元';
                }
            }
        }
//获取按照年收费的样式事件元素
...
//结束付款
 ...
 // //点击退出
 // var logininuser= document.querySelector('#login-in-user');
 // var loginoutuser= document.querySelector('#login-out-user');
 // logininuser.onclick=function(){
 // 	loginoutuser.style.display='block';
 // }
// function hidelogin(){
